...
class GroupedListViewWidget<T, E> extends StatefulWidget {
  ...
  /// Creates a [GroupedListViewWidget]
  const GroupedListViewWidget({
    Key? key,
    ...
    this.separator = const SizedBox.shrink(),
        ...
        assert(groupSeparatorBuilder != null),
        super(key: key);

  @override
  State<StatefulWidget> createState() => _GroupedListViewWidgetState<T, E>();
}

class _GroupedListViewWidgetState<T, E>
    extends State<GroupedListViewWidget<T, E>> {
  ...
  Widget build(BuildContext context) {
    ...
    return SliverList(
      ...
      if (_isSeparator(index)) {
        var current = widget.groupBy(_sortedElements[actualIndex]);
        var previous = widget.groupBy(_sortedElements[actualIndex - 1]);
        if (previous != current) {
